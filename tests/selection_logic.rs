...
fn test_selection_sorting_and_budget() {
    ...
    // 2. Test Budget Cutoff
    // Budget 2.
    // A (1) fits. Rem: 1.
    // B (2) too big for 1. Skip.
    // C (1) fits in 1. Rem: 0.
    // Result: A, C.
    
    let result_constrained = selector.select(&cache, query, 2).unwrap();
    
    let ids: Vec<String> = result_constrained.documents.iter().map(|d| d.id.clone()).collect();
